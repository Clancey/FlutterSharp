...
public class ValidationService
{
	...
	{
		// Check for duplicate widget names - treat as warnings since they're skipped during generation
		var widgetNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
		foreach (var widget in package.Widgets)
		{
			if (!widgetNames.Add(widget.Name))
			{
				// Duplicates are warnings, not errors - they'll be skipped during generation
				// This commonly happens with internal Flutter widgets or platform-specific widgets
				result.Warnings.Add(new ValidationWarning($"Widget '{widget.Name}'", "Duplicate widget name (case-insensitive) - will be skipped"));
			}
		}
