	...
	public class DartAnalyzerHost
	{
		...
		public async Task<bool> ValidateDartToolsAsync(CancellationToken cancellationToken = default)
		{
			...
			if (scriptDirectory != null)
			{
				...
				{
					await EnsureDartDependenciesAsync(scriptDirectory, cancellationToken);
				}
			}
			...
			CancellationToken cancellationToken = default)
		{
			...
			var analyzerScriptFullPath = Path.GetFullPath(_analyzerScriptPath);
			var scriptDirectory = Path.GetDirectoryName(analyzerScriptFullPath)
				?? throw new DartAnalyzerException($"Could not determine script directory for analyzer script: {analyzerScriptFullPath}");
			var scriptFileName = Path.GetFileName(analyzerScriptFullPath);

			await EnsureDartDependenciesAsync(scriptDirectory, cancellationToken);

			// Run from the analyzer script directory so pubspec.lock and package_config are respected.
			var arguments = $"run \"{scriptFileName}\" \"{fullPath}\" \"{includeArg}\" \"{excludeArg}\"";

			try
			{
				var jsonOutput = await ExecuteCommandAsync("dart", arguments, cancellationToken, scriptDirectory);
		...
		/// <summary>
		/// Ensures Dart dependencies for the analyzer tool are installed.
		/// </summary>
		private async Task EnsureDartDependenciesAsync(string scriptDirectory, CancellationToken cancellationToken)
		{
			var pubspecPath = Path.Combine(scriptDirectory, "pubspec.yaml");
			if (!File.Exists(pubspecPath))
			{
