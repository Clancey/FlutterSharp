...
namespace FlutterSharp.CodeGen.Generators.Dart
{
	...
	public class DartParserGenerator
	{
		...
		/// <param name="dartOutputDirectory">The Dart output directory that contains the generated struct files.</param>
		public DartParserGenerator(CSharpToDartFfiMapper typeMapper, string dartOutputDirectory)
		{
			_typeMapper = typeMapper ?? throw new ArgumentNullException(nameof(typeMapper));
			_dartStructsDirectory = Path.Combine(
				Path.GetFullPath(dartOutputDirectory ?? throw new ArgumentNullException(nameof(dartOutputDirectory))),
				"structs");
			...
			if (widget.ChildPropertyName != null)
			{
				...
				if (childProperty != null)
				{
					childIsNullable = childProperty.IsNullable
						|| childProperty.DartType?.EndsWith("?", StringComparison.Ordinal) == true;
				}
			}
			...
			var model = new
			{
				...
				Properties = regularProperties.Select(p => {
					...
					return new
					{
						...
						HasFieldName = p.IsNullable ? ToHasFieldName(p.Name) : null,
						DartType = p.DartType,
						...
						IsWidgetList = baseType.StartsWith("List<Widget", StringComparison.Ordinal)
							&& (ffiType == "Pointer<ChildrenStruct>" || ffiType == "IntPtr"),
				...
				ChildHasFieldName = widget.HasSingleChild && !widget.HasMultipleChildren && childPropName != null && childIsNullable
					? ToHasFieldName(childPropName) : null,
	...
	public string Generate(EnrichedWidgetDefinition enrichedWidget)
	{
		...
		if (enrichedWidget.ChildPropertyName != null)
		{
			...
			if (childProperty != null)
			{
				childIsNullable = childProperty.IsDartNullable || childProperty.IsNullable;
			}
		}
		...
		var model = new
		{
			...
			ChildPropertyName = enrichedWidget.HasSingleChild && !enrichedWidget.HasMultipleChildren && childPropName != null
				? ToCamelCase(childPropName) : null,
			ChildHasFieldName = enrichedWidget.HasSingleChild && !enrichedWidget.HasMultipleChildren && childPropName != null && childIsNullable
				? ToHasFieldName(childPropName) : null,
			child_is_nullable = childIsNullable,
			// Only set ChildrenPropertyName for multi-child widgets (not single-child)
			// Use overridden childrenPropName which may be "slivers" instead of "children" for viewport widgets
			ChildrenPropertyName = enrichedWidget.HasMultipleChildren && childrenPropName != null
				? ToCamelCase(childrenPropName) : null,
			SourceLibrary = enrichedWidget.SourceLibrary,
	...
	private string GetParserFunctionFromEnriched(EnrichedPropertyDefinition property)
	{
		...
		private string ToHasFieldName(string value)
		{
			...
			return $"has{char.ToUpperInvariant(camelCase[0])}{camelCase.Substring(1)}";
		}
