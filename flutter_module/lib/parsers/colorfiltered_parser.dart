// <auto-generated>
// This file was automatically generated on 2026-04-09 02:46:15 UTC
// DO NOT EDIT - Changes will be overwritten
// </auto-generated>

import 'dart:ffi' hide Size;
import 'package:flutter/widgets.dart';
import 'package:flutter_module/flutter_sharp_structs.dart';
import 'package:flutter_module/maui_flutter.dart';
import '../generated/structs/colorfiltered_struct.dart';

/// Applies a [ColorFilter] to its child.
///
/// This widget applies a function independently to each pixel of [child]'s
/// content, according to the [ColorFilter] specified.
/// Use the [ColorFilter.mode] constructor to apply a [Color] using a [BlendMode].
/// Use the [BackdropFilter] widget instead, if the [ColorFilter]
/// needs to be applied onto the content beneath [child].
///
/// {@youtube 560 315 https://www.youtube.com/watch?v=F7Cll22Dno8}
///
/// {@tool dartpad}
/// These two images have two [ColorFilter]s applied with different [BlendMode]s,
/// one with red color and [BlendMode.modulate] another with a grey color and [BlendMode.saturation].
///
/// ** See code in examples/api/lib/widgets/color_filter/color_filtered.0.dart **
/// {@end-tool}
///
/// See also:
///
/// * [BlendMode], describes how to blend a source image with the destination image.
/// * [ColorFilter], which describes a function that modify a color to a different color.
class ColorFilteredParser extends WidgetParser {
  @override
  Widget parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    var map = Pointer<ColorFilteredStruct>.fromAddress(fos.handle.address).ref;
    return ColorFiltered(
        // TODO: colorFilter uses Pointer<Void> and cannot be marshaled through FFI.
        // We only pass a placeholder when a non-null pointer is provided.
        colorFilter: map.colorFilter.address != 0
            ? _getColorFilterPlaceholder()
            : (null as dynamic),
        child: map.hasChild != 0
            ? DynamicWidgetBuilder.buildFromPointer(map.child, buildContext)
            : null);
  }

  @override
  String get widgetName => "ColorFiltered";

  // Placeholder methods for parameters that cannot be marshaled through FFI.
  // TODO: Implement proper handling for these parameters or create a manual wrapper
  dynamic _getColorFilterPlaceholder() {
    // TODO: Provide proper default value for ColorFilter
    throw UnimplementedError(
        'ColorFiltered.colorFilter requires manual implementation - type ColorFilter cannot be auto-generated');
  }
}
