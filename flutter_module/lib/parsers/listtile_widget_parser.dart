...
import '../generated/structs/listtile_struct.dart';
import '../utils.dart';
...
class ListTileWidgetParser extends WidgetParser {
  ...
  Widget? parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    ...
    final enableFeedback = map.hasEnableFeedback == 1 ? map.enableFeedback == 1 : null;

    // Parse callback action IDs
    final onTapAction = map.onTapAction.address != 0
    ...
    final tileColor = map.hasTileColor == 1
        ? Color(map.tileColor)
        : null;
    final selectedTileColor = map.hasSelectedTileColor == 1
        ? Color(map.selectedTileColor)
        : null;
    final iconColor = map.hasIconColor == 1
        ? Color(map.iconColor)
        : null;
    final textColor = map.hasTextColor == 1
        ? Color(map.textColor)
        : null;
    final focusColor = map.hasFocusColor == 1
        ? Color(map.focusColor)
        : null;
    final hoverColor = map.hasHoverColor == 1
        ? Color(map.hoverColor)
        : null;
    final splashColor = map.hasSplashColor == 1
        ? Color(map.splashColor)
        : null;

    // Parse numeric properties
    final horizontalTitleGap = map.hasHorizontalTitleGap == 1
        ? map.horizontalTitleGap
        : null;
    final minVerticalPadding = map.hasMinVerticalPadding == 1
        ? map.minVerticalPadding
        : null;
    final minLeadingWidth = map.hasMinLeadingWidth == 1
        ? map.minLeadingWidth
        : null;
    final minTileHeight = map.hasMinTileHeight == 1
        ? map.minTileHeight
        : null;

    // Parse enum properties (sentinel value -1 means null)
    ListTileTitleAlignment? titleAlignment;
    if (map.titleAlignment >= 0 && map.titleAlignment < ListTileTitleAlignment.values.length) {
