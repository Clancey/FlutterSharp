...
class CupertinoSwitchParser extends WidgetParser {
  ...
  Widget? parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    final map = Pointer<CupertinoSwitchStruct>.fromAddress(fos.handle.address).ref;
    ...
    if (map.hasTrackOutlineColor == 1) {
      trackOutlineColor = Color(map.trackOutlineColor);
    }
    ...
    // Parse track outline width
    double? trackOutlineWidth;
    if (map.hasTrackOutlineWidth == 1) {
      trackOutlineWidth = map.trackOutlineWidth;
    }

    // Parse applyTheme
    bool? applyTheme;
    if (map.hasApplyTheme == 1) {
      applyTheme = map.applyTheme == 1;
    }
    ...
    return CupertinoSwitch(
      ...
      onChanged: onChangedAction != null
          ? (newValue) => _invokeActionWithArgs(onChangedAction, {'value': newValue})
          : null,
