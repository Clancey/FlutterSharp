...
import '../generated/structs/bottomnavigationbar_struct.dart';
import '../generated/structs/bottomnavigationbaritem_struct.dart';
import '../utils.dart';
...
class BottomNavigationBarParser extends WidgetParser {
  ...
  Widget? parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    final map = Pointer<BottomNavigationBarStruct>.fromAddress(fos.handle.address).ref;
    ...
    if (map.landscapeLayout >= 0 && map.landscapeLayout < BottomNavigationBarLandscapeLayout.values.length) {
      landscapeLayout = BottomNavigationBarLandscapeLayout.values[map.landscapeLayout];
    }
  ...
  BottomNavigationBarItem? _parseBottomNavigationBarItem(
      BottomNavigationBarItemStruct itemStruct,
      BuildContext buildContext) {
    // Parse icon (required)
    Widget? icon;
    if (itemStruct.icon.address != 0) {
      icon = DynamicWidgetBuilder.buildFromPointer(itemStruct.icon, buildContext);
    }
    if (icon == null) {
    ...
    if (itemStruct.activeIcon.address != 0) {
      activeIcon = DynamicWidgetBuilder.buildFromPointer(itemStruct.activeIcon, buildContext);
    }
