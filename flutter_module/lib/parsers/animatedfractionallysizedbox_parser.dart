// <auto-generated>
// This file was automatically generated on 2026-01-08 22:03:24 UTC
// DO NOT EDIT - Changes will be overwritten
// </auto-generated>

import 'dart:ffi' hide Size;
import 'package:ffi/ffi.dart';
import 'package:flutter/widgets.dart';
import 'package:flutter_module/flutter_sharp_structs.dart';
import '../generated/generated_utility_parsers.dart';
import 'package:flutter_module/maui_flutter.dart';
import '../generated/structs/animatedfractionallysizedbox_struct.dart';

/// Animated version of [FractionallySizedBox] which automatically transitions the
/// child's size over a given duration whenever the given [widthFactor] or
/// [heightFactor] changes, as well as the position whenever the given [alignment]
/// changes.
///
/// For the animation, you can choose a [curve] as well as a [duration] and the
/// widget will automatically animate to the new target [widthFactor] or
/// [heightFactor].
///
/// {@tool dartpad}
/// The following example transitions an [AnimatedFractionallySizedBox]
/// between two states. It adjusts the [heightFactor], [widthFactor], and
/// [alignment] properties when tapped, using a [curve] of [Curves.fastOutSlowIn]
///
/// ** See code in examples/api/lib/widgets/implicit_animations/animated_fractionally_sized_box.0.dart **
/// {@end-tool}
///
/// See also:
///
/// * [AnimatedAlign], which is an implicitly animated version of [Align].
/// * [AnimatedContainer], which can transition more values at once.
/// * [AnimatedSlide], which can animate the translation of child by a given offset relative to its size.
/// * [AnimatedPositioned], which, as a child of a [Stack], automatically
/// transitions its child's position over a given duration whenever the given
/// position changes.
class AnimatedFractionallySizedBoxParser extends WidgetParser {
  @override
  Widget parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    var map = Pointer<AnimatedFractionallySizedBoxStruct>.fromAddress(
            fos.handle.address)
        .ref;
    return AnimatedFractionallySizedBox(
        heightFactor: map.heightFactor,
        widthFactor: map.widthFactor,
        // TODO: alignment is required but type 'AlignmentGeometry' cannot be marshaled through FFI
        // This widget needs a manual wrapper to properly handle this parameter
        alignment: _getAlignmentPlaceholder(),
        curve:
            map.curve.address != 0 ? parseCurve(map.curve.ref) : Curves.linear,
        duration: parseDurationMicroseconds(map.duration),
        onEnd: createVoidCallback(map.onEndAction.address != 0
            ? map.onEndAction.toDartString()
            : null),
        child: DynamicWidgetBuilder.buildFromPointerNotNull(
            map.child, buildContext));
  }

  @override
  String get widgetName => "AnimatedFractionallySizedBox";

  // Placeholder methods for required parameters that cannot be marshaled through FFI
  // TODO: Implement proper handling for these parameters or create a manual wrapper
  dynamic _getAlignmentPlaceholder() {
    // TODO: Provide proper default value for AlignmentGeometry
    throw UnimplementedError(
        'AnimatedFractionallySizedBox.alignment requires manual implementation - type AlignmentGeometry cannot be auto-generated');
  }
}
