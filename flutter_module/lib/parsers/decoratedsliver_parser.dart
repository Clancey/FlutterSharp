// <auto-generated>
// This file was automatically generated on 2026-04-09 02:46:15 UTC
// DO NOT EDIT - Changes will be overwritten
// </auto-generated>

import 'dart:ffi' hide Size;
import 'package:flutter/widgets.dart';
import 'package:flutter_module/flutter_sharp_structs.dart';
import '../generated/generated_utility_parsers.dart';
import 'package:flutter_module/maui_flutter.dart';
import '../generated/structs/decoratedsliver_struct.dart';

/// A sliver widget that paints a [Decoration] either before or after its child
/// paints.
///
/// Unlike [DecoratedBox], this widget expects its child to be a sliver, and
/// must be placed in a widget that expects a sliver.
///
/// If the child sliver has infinite [SliverGeometry.scrollExtent], then we only
/// draw the decoration down to the bottom [SliverGeometry.cacheExtent], and
/// it is necessary to ensure that the bottom border does not creep
/// above the top of the bottom cache. This can happen if the bottom has a
/// border radius larger than the extent of the cache area.
///
/// Commonly used with [BoxDecoration].
///
///
/// {@tool dartpad}
/// This sample shows a radial gradient that draws a moon on a night sky:
///
/// ** See code in examples/api/lib/widgets/sliver/decorated_sliver.0.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This example demonstrates how the [CustomScrollView.clipBehavior]
/// impacts a decorated sliver's appearance.
///
/// The [Switch] determines whether clipping is enabled, and
/// the [Slider] adjusts the height of window.
///
/// ** See code in examples/api/lib/widgets/sliver/decorated_sliver.1.dart **
/// {@end-tool}
///
/// This widget does not apply any additional clipping to its [child].
/// To clip a child based on the [Decoration]'s shape, consider using
/// a [ClipPath] widget.
///
/// See also:
///
/// * [DecoratedBox], the version of this class that works with RenderBox widgets.
/// * [Decoration], which you can extend to provide other effects with
/// [DecoratedSliver].
/// * [CustomPaint], another way to draw custom effects from the widget layer.
class DecoratedSliverParser extends WidgetParser {
  @override
  Widget parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    var map =
        Pointer<DecoratedSliverStruct>.fromAddress(fos.handle.address).ref;
    return DecoratedSliver(
        decoration: map.decoration.address != 0
            ? parseDecoration(map.decoration.ref)
            : const BoxDecoration(),
        // TODO: position uses Pointer<Void> and cannot be marshaled through FFI.
        // We only pass a placeholder when a non-null pointer is provided.
        position: map.position.address != 0
            ? _getPositionPlaceholder()
            : (null as dynamic),
        sliver: DynamicWidgetBuilder.buildFromPointerNotNull(
            map.sliver, buildContext));
  }

  @override
  String get widgetName => "DecoratedSliver";

  // Placeholder methods for parameters that cannot be marshaled through FFI.
  // TODO: Implement proper handling for these parameters or create a manual wrapper
  dynamic _getPositionPlaceholder() {
    // TODO: Provide proper default value for DecorationPosition
    throw UnimplementedError(
        'DecoratedSliver.position requires manual implementation - type DecorationPosition cannot be auto-generated');
  }
}
