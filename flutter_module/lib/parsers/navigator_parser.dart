...
class NavigatorParser extends WidgetParser {
  ...
  Widget? parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    ...
    // Parse initial route
    final initialRoute =
        map.hasInitialRoute == 1 ? parseString(map.initialRoute) ?? '/' : '/';
    ...
    // Parse navigator ID
    final navigatorId =
        map.hasNavigatorId == 1 ? parseString(map.navigatorId) : null;

    // Parse clip behavior
    final clipBehavior =
        Clip.values[map.clipBehavior.clamp(0, Clip.values.length - 1)];
    ...
    final onPopAction =
        map.hasOnPopAction == 1 ? parseString(map.onPopAction) : null;
    ...
    final transitionDurationMs =
        map.transitionDurationMs > 0 ? map.transitionDurationMs : 300;
    final reverseTransitionDurationMs = map.reverseTransitionDurationMs > 0
        ? map.reverseTransitionDurationMs
        : 300;
    ...
    if (map.currentChild.address != 0) {
      currentChild =
          DynamicWidgetBuilder.buildFromPointer(map.currentChild, buildContext);
    }
    ...
    if (map.previousChild.address != 0 && isTransitioning) {
      previousChild = DynamicWidgetBuilder.buildFromPointer(
          map.previousChild, buildContext);
    }
...
class _NavigatorWrapperState extends State<_NavigatorWrapper>
  ...
  void _initAnimationController() {
    ...
    if (widget.isTransitioning &&
        widget.transitionType != RouteTransitionType.none) {
      ...
      builder: (context, child) {
        return Stack(
          children: [
            ...
            _buildAnimatedChild(widget.currentChild,
                isIncoming: !widget.isPopping),
          ],
        );
      },
    );
  ...
  Future<void> _invokePopAction(String actionId, String routeName) async {
    try {
      await raiseMauiEvent(actionId, 'invoke', {'routeName': routeName});
    } catch (e) {
