...
class RefreshIndicatorParser extends WidgetParser {
  ...
  Widget? parse(IFlutterObjectStruct fos, BuildContext buildContext) {
    final map = Pointer<RefreshIndicatorStruct>.fromAddress(fos.handle.address).ref;
    ...
    final onRefreshAction = map.hasOnRefresh == 1
        ? parseString(map.onRefreshAction)
        : null;
    ...
    if (map.triggerMode >= 0 && map.triggerMode < RefreshIndicatorTriggerMode.values.length) {
  ...
  Future<void> _createRefreshFuture(String widgetId, String actionId) {
    ...
    return completer.future.timeout(
      ...
      onTimeout: () {
        ...
        debugPrint('RefreshIndicator: onRefresh timed out for widget $widgetId');
      },
    );
  }
