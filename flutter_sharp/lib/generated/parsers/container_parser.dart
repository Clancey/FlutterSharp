...
/// Parser that converts ContainerStruct to Container widget.
class ContainerParser implements WidgetParser {
  @override
  Widget parse(Pointer<Void> structPtr) {
    if (structPtr.address == 0) {
      throw ArgumentError('Null pointer passed to ContainerParser');
    }

    final struct = structPtr.cast<ContainerStruct>().ref;

    return Container(
      child: struct.child.address != 0
        ? parseWidget(struct.child)
        : null,      alignment: parseObject(struct.alignment),      padding: parseObject(struct.padding),      color: parseObject(struct.color),      decoration: parseObject(struct.decoration),      foregroundDecoration: parseObject(struct.foregroundDecoration),      constraints: parseObject(struct.constraints),      margin: parseObject(struct.margin),      transform: parseObject(struct.transform),      transformAlignment: parseObject(struct.transformAlignment),      clipBehavior: parseObject(struct.clipBehavior),      child: parseChild(struct.child)
    );
  }
