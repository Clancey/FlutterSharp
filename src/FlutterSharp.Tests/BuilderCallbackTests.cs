...
namespace FlutterSharp.Tests
{
	...
	public class BuilderCallbackTests
	{
		...
		[Test]
		public void EventMessage_InvalidJson_DoesNotInvokeHandlers()
		{
			// Arrange
			var handlerInvoked = false;
			FlutterManager.RegisterEventHandler("BadJsonEvent", (_, _, _) => handlerInvoked = true);

			// Act / Assert
			Assert.DoesNotThrow(() =>
				Communicator.OnCommandReceived?.Invoke(("Event", "{not-valid-json", _ => { })));
			Assert.That(handlerInvoked, Is.False);
		}

		[Test]
		public void EventMessage_MissingEventName_DoesNotInvokeHandlers()
		{
			// Arrange
			var handlerInvoked = false;
			FlutterManager.RegisterEventHandler("TestEvent", (_, _, _) => handlerInvoked = true);
		...
		[Test]
		public void HandleAction_InvalidActionId_DoesNotInvokeCallback()
		{
			// Arrange
			var callbackInvoked = false;
			var callbackId = CallbackRegistry.Register((Action)(() => callbackInvoked = true));
			var actionJson = JsonSerializer.Serialize(new
			{
				actionId = "action_not-a-number"
			});
			string? response = null;
		...
		[Test]
		public void HandleAction_CancelledByInvokingFilter_ReturnsCancelledResponse()
		{
			// Arrange
			var callbackInvoked = false;
			var callbackId = CallbackRegistry.Register((Action)(() => callbackInvoked = true));
			...
			void CancelInvocation(object? _, ActionInvokingEventArgs args) => args.Cancel = true;
			FlutterManager.OnActionInvoking += CancelInvocation;
			...
			// Assert
			Assert.That(callbackInvoked, Is.False);
			Assert.That(response, Does.Contain("\"cancelled\": true"));
		}

		#endregion

		#region Invoke Round-Trip Tests

		[Test]
		public async Task InvokeMethodAsync_ResponseRoundTrip_ReturnsResult()
		{
			// Arrange
			Communicator.SendCommand = cmd =>
			{
				if (cmd.Method != "Invoke")
					return;

				var invokePayload = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(cmd.Arguments)!;
				var requestId = invokePayload["requestId"].GetInt32();
				...
				Communicator.OnCommandReceived?.Invoke(("InvokeResponse", responseJson, _ => { }));
			};

			// Act
			var result = await Communicator.InvokeMethodAsync("Ping", new { value = 1 }, TimeSpan.FromSeconds(1));

			// Assert
			Assert.That(result, Is.EqualTo("pong"));
			Assert.That(Communicator.PendingRequests, Is.EqualTo(0));
		}

		[Test]
		public async Task InvokeMethodAsync_ErrorRoundTrip_ReturnsNull()
		{
			// Arrange
			Communicator.SendCommand = cmd =>
			{
				...
				Communicator.OnCommandReceived?.Invoke(("InvokeError", responseJson, _ => { }));
			};

			// Act
			var result = await Communicator.InvokeMethodAsync("Ping", new { value = 2 }, TimeSpan.FromSeconds(1));
