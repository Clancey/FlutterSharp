...
namespace Flutter
{
    ...
    public static class NotifierRegistry
    {
        ...
        public static bool HandleStateNotify(string notifierId, object value, string sourceWidgetId)
        {
            ...
            if (notifier == null)
            {
                Console.WriteLine($"NotifierRegistry: No notifier found with ID '{notifierId}'");
                return false;
            }
    ...
    public class BidirectionalNotifier<T> : ValueNotifier<T>, IBidirectionalNotifier
    {
        ...
        private void BroadcastToDart()
        {
            ...
            {
                Console.WriteLine($"BidirectionalNotifier: Error broadcasting to Dart: {ex.Message}");
            }
        }
        ...
        public bool UpdateFromDart(object value, string sourceWidgetId)
        {
            lock (_updateLock)
            {
                ...
                {
                    Console.WriteLine($"BidirectionalNotifier: Error updating from Dart: {ex.Message}");
                    return false;
                }
                finally
                {
