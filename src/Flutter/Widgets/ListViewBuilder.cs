...
namespace Flutter.Widgets
{
	...
	public class ListViewBuilder : Widget
	{
		...
		public ListViewBuilder(int itemCount, Func<int, Widget> itemBuilder)
		{
		...
		public int ItemCount
		{
			...
			{
				_itemCount = value;
				if (backingStruct is ListViewBuilderStruct s)
				{
					s.itemCount = value;
				}
			}
		...
		public override void SendEvent(string eventName, string data, Action<string>? callback = null)
		{
			if (eventName == "ItemBuilder" && _itemBuilder != null && callback != null)
			{
				try
				{
					...
					{
						Console.WriteLine($"ListViewBuilder: Index {index} out of range (0-{_itemCount - 1})");
				...
				{
					Console.WriteLine($"ListViewBuilder: Error building item at index: {ex}");
