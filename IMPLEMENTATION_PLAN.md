...
| INP005 | Implement DropdownButton | completed | C# widget/struct (DropdownButton.cs, DropdownButtonStruct.cs, DropdownMenuItem.cs, DropdownMenuItemStruct.cs) + Dart struct/parser with items array, value, onChanged, hint, disabledHint, onTap, elevation, underline, icon, iconSize, isDense, isExpanded, itemHeight, colors (iconDisabled, iconEnabled, focus, dropdown), menuMaxHeight, enableFeedback, alignment, borderRadius, padding. Added BorderRadius and AlignmentDirectional types to PlaceholderTypes.cs |
...
| LST002 | Ensure GridView works | completed | Rewrote GridViewStruct.cs (proper Has* flags), GridView.cs (collection initializers, properties), created gridview_struct.dart FFI struct, fixed gridview_widget_parser.dart |
...
- [x] List widgets work with items (ListView, GridView, ListTile) ✅
...
| SCR004 | Implement RefreshIndicator | completed | RefreshIndicator.cs with Func<Task> onRefresh callback. RefreshIndicatorStruct with displacement, edgeOffset, color, backgroundColor, strokeWidth, triggerMode. refreshindicator_parser.dart with async callback completion using Completer pattern. FlutterManager.SendAsyncCallbackComplete signals Future completion to Dart. Async callback pattern enables C# async/await to control Dart Future completion. |
| SCR005 | Implement infinite scrolling pattern | completed | ScrollController extended with LoadMoreThreshold, OnLoadMore callback, IsLoadingMore state, and LoadMoreRequested event. InfiniteListView and InfiniteGridView convenience widgets combine ListView.builder/GridView.builder with automatic load-more detection. C# side detects when scroll position is within threshold of maxScrollExtent and triggers async OnLoadMore callback. Dart parsers (infinitelistview_parser.dart, infinitegridview_parser.dart) handle item building via FutureBuilder pattern with automatic loading indicator at end. InfiniteListViewStruct/InfiniteGridViewStruct for FFI. |
...
| CUP005 | Implement CupertinoSwitch | completed | iOS-style switch with value, onChanged callback, color properties (activeTrackColor, inactiveTrackColor, thumbColor, trackOutlineColor, focusColor), trackOutlineWidth, focus handling (onFocusChange, autofocus), applyTheme, dragStartBehavior |
